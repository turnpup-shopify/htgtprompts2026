...
export async function getRoomOptions() {
  ...
  if (masterRows.length) {
    ...
    for (const row of masterRows) {
      ...
      if (scene) {
        const entry = roomMap.get(roomKey);
        if (!entry.scenes.includes(scene)) entry.scenes.push(scene);

        if (!sceneToRooms.has(scene)) sceneToRooms.set(scene, []);
        const rooms = sceneToRooms.get(scene);
        if (!rooms.includes(roomLabel)) rooms.push(roomLabel);
      }
    }

    const roomOptions = [...roomMap.values()].sort((a, b) =>
      String(a.roomType).localeCompare(String(b.roomType))
    );

    const sceneOptions = [...sceneToRooms.entries()]
      .map(([scene, rooms]) => ({ scene, rooms }))

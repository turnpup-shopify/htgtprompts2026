...
export async function getLocalImageCatalogByFurnitureType(furnitureTypes = []) {
  ...
  const configuredBlobPrefix = getBlobPrefix();
  let resolvedBlobPrefix = configuredBlobPrefix;
  let allImages;

  if (storageSource === 'blob') {
    allImages = await listBlobImages(configuredBlobPrefix);

    // If no files were found under configured prefix (default: items/),
    // fall back to listing the blob root so folders like "accent table/"
    // still work without extra config.
    if (!allImages.length && configuredBlobPrefix) {
      allImages = await listBlobImages('');
      resolvedBlobPrefix = '';
    }

    allImages.sort((a, b) => a.relativePath.localeCompare(b.relativePath));
  } else {
    allImages = (await collectImagesRecursive(imageDir)).sort((a, b) =>
  ...
  return {
    storageSource,
    imageDir,
    blobPrefix: resolvedBlobPrefix,
    configuredBlobPrefix,

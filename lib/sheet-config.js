  ...
  return records
    .map((record) => {
      ...
      return {
        ...
        scene: String(pickHeader(record, ['scene']) || '').trim()
      };
    })
    .filter(Boolean);
}
...
export function findMasterRowByRoom(roomType, rows = [], scene = '') {
  ...
  if (normalizedScene) {
    const sceneExact = rows.find(
      (row) => row.room_key === roomKey && row.scene.toLowerCase() === normalizedScene
    );
    if (sceneExact) return sceneExact;

    const scenePartial = rows.find(
      (row) =>
        (row.room_key.includes(roomKey) || roomKey.includes(row.room_key)) &&
        row.scene.toLowerCase() === normalizedScene
    );
    if (scenePartial) return scenePartial;
  }

  const exact = rows.find((row) => row.room_key === roomKey && !row.scene);
  if (exact) return exact;

  const exactAny = rows.find((row) => row.room_key === roomKey);
